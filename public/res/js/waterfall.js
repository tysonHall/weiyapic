$(function () {
    var wrap = $('.part-2'),
        ...
        col = [],
        total = 0,
        mt = $('.part-2 .pic-3')[0].offsetHeight,
        detail = $('.part-2 .show-detail div'),
        origin = $('.part-2 .show-origin div'),
        btnOrigin = $('.part-2 .show-detail button'),
        btnClose = $('.part-2 .show-origin button'),
        curImg = {},
        tip = $('.part-2 .tip'),
        toptip = $('.part-2 .toptip'),
        haveMore0 = true;
    ...
    wrap.on('touchmove', function (e) {
        ...
        if(haveMore0 && movey - tStart.y <= -10)
        {
    ...
    wrap.on('touchend', function (e) {
        ...
        if (tEnd.y - tStart.y <= -10 && haveMore0) {   // 上下拉
            if ($(window).scrollTop() >= $('.part-2.main').height() - winH * 1.2) {
                loadMore();
                // tip.hide();
            }
        }
    ...
    function loadMore() {
        ...
        $.post('/index/myfile/ajax_file',{'typeid':typeid,'num':num[0],'count':count,'table':table},function(res){
			if(res.length == 0) {
				 haveMore0 = false;
            ...
            for (var i = 0; i < urlArr.length; i++) {
				...
				{
					boxtop = spacing+height0[0];
					boxleft = 0;
					height0[0] += urlArr[i].height * wfcW / urlArr[i].width + spacing;
				}
				else
				{
					boxtop = spacing+height1[0];
					boxleft = wfcW + spacing;
					height1[0] += urlArr[i].height * wfcW / urlArr[i].width + spacing;
				}
    ...
    function get_newimg()
    {
        $.post('/index/myfile/ajax_newimg', {'maxid':maxid[0],'typeid':typeid,'table':table},function(res){
            if(res > 0)
            {
                wfp.html('');
                num[0] = 0;
    ...
    function showDetail() {
        wfc.live('click', function () {
			...
			if (!detail.find('img').hasClass('animate')) {
                transformScale($('.part-2 .show-detail img')[0], true);
			}
            else
            {
                resettransform($('.part-2 .show-detail img')[0]);
            }
        });
        ...
        btnOrigin.live('click', function () {
            origin.parent().show();
            $('body').removeClass('ovf');
            transformScale($('.part-2 .show-origin img')[0], false);
        });
    ...
    function transformScale(el, isMove) {
        ...
        //单点拖动
        function onPan(ev) {
            // console.log((el.offsetWidth*transform.scale+transform.translate.x), '-', document.body.offsetWidth, '=', (el.offsetWidth*transform.scale+transform.translate.x)-document.body.offsetWidth);
            el.className = '';
            transform.translate = {
                x: END_X + ev.deltaX,
